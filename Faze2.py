...
def tf_idf_document() :
    ...
    for word in dictionary :
        ...
        for key in word.docTerms.keys() :
            ...
            weight = tf * idf 
            list_of_doc_weights.append(weight)
            pair = (word.id , weight)
...
def length_document(doc_vector) : 
    # print(doc_vector)
    length = []
    sum_of_term_weight = 0
    # makhraj kasr baraye mohasebeye length
    for i in range(len(doc_vector)) : 
        doc_i = doc_vector[i]
        # print(i)
        for term_weight in doc_i : 
            # print(term_weight[0], term_weight[1])
            power_of_term_weight = term_weight[1] * term_weight[1]
            sum_of_term_weight += power_of_term_weight
        sqrt = math.sqrt(sum_of_term_weight)
        length.append(sqrt)
    return length
...
query = tf_idf_query()
doc_vector = tf_idf_document()
length = length_document(doc_vector)
cosine_similarity(query, length, doc_vector)
